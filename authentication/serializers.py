...
class RegisterSerializer(serializers.ModelSerializer):
    ...
    def validate(self, attrs):
            ...
            raise serializers.ValidationError(
                {
                    "verification_status": {
                        ...
                        "next_step": next_step,
                        "expires_in": {"minutes": minutes, "seconds": seconds},
                    }
                }
            )
        ...
        if User.objects.filter(
            phone_number=phone_number, registration_pending=False
        ).exists():
            raise serializers.ValidationError(
                {
...
class VerifyEmailOTPSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        otp_instance = (
            OTP.objects.filter(
                user=user, code=attrs["email_otp"], type="EMAIL", is_verified=False
            )
            .order_by("-created_at")
            .first()
        )
...
class VerifyPhoneOTPSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        otp_instance = (
            OTP.objects.filter(
                user=user, code=attrs["phone_otp"], type="PHONE", is_verified=False
            )
            .order_by("-created_at")
            .first()
        )
...
class VerifyOTPSerializer(serializers.Serializer):
    ...
    def validate(self, attrs):
        ...
        try:
            ...
            otp_instance = (
                OTP.objects.filter(
                    user=user, is_verified=False  # Ensures we get an unverified OTP
                )
                .order_by("-created_at")
                .first()
            )
